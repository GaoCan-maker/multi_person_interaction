...
def plot_3d_motion_multi(
    ...
    # 显式指定 writer 来避免 MovieWriterRegistry 迭代器错误
    try:
        writer = FFMpegFileWriter(fps=fps)
        ani.save(save_path, writer=writer)
    except Exception as e:
        # 如果 FFMpegFileWriter 失败，尝试使用字符串指定 writer
        print(f"Warning: FFMpegFileWriter failed ({e}), trying with writer='ffmpeg'")
        try:
            ani.save(save_path, fps=fps, writer='ffmpeg')
        except Exception as e2:
            # 如果还是失败，尝试使用 pillow writer（可能需要将扩展名改为 .gif）
            print(f"Warning: ffmpeg writer failed ({e2}), trying with writer='pillow'")
            if save_path.endswith('.mp4'):
                save_path_gif = save_path.replace('.mp4', '.gif')
                print(f"Saving as GIF instead: {save_path_gif}")
                ani.save(save_path_gif, fps=fps, writer='pillow')
            else:
