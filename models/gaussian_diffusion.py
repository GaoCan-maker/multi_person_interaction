...
class MotionDiffusion(GaussianDiffusion):
    ...
    def __init__(self, use_timesteps, motion_rep, **kwargs):
        ...
        self.t_bar = 700
        # Range-guidance stabilization: clamp noisy LLM ranges into a safe band
        # and use a soft margin to reduce oscillatory gradients near boundaries.
        self.pair_dist_global_min = 0.2
        self.pair_dist_global_max = 3.5
        self.pair_dist_min_span = 0.15
        self.pair_dist_soft_margin = 0.05
        self.pair_dist_violation_cap = 1.0
        ...
        for i in indices:
            ...
            for j in range(n_person):
                with th.no_grad():
                    ...
                    if len(inter_graph['in'][j]) == 0:
                        model_kwargs['b'] = th.zeros_like(imgs[j])
                        sampling_info['unrelated'] = []
                        sampling_info['unrelated_idx'] = []
                        sampling_info['out_unrelated'] = []
                        sampling_info['cond'] = th.zeros_like(starts[j])
                        sampling_info['current_idx'] = j
                        sampling_info['all_starts'] = starts
                        sampling_info['pair_dists'] = inter_graph.get("pair_dists", None)
                    ...
                    for k, cond_index in enumerate(inter_graph['in'][j]):  # Loop through condition characters
                        ...
                        sampling_info['unrelated'] = []
                        sampling_info['unrelated_idx'] = unrelated_index
                        sampling_info['out_unrelated'] = []
    ...
    def _sanitize_pair_range(self, d_min, d_max):
        if d_min is None and d_max is None:
            return None, None

        if d_min is None:
            d_min = self.pair_dist_global_min
        if d_max is None:
            d_max = self.pair_dist_global_max

        d_min = float(np.clip(d_min, self.pair_dist_global_min, self.pair_dist_global_max))
        d_max = float(np.clip(d_max, self.pair_dist_global_min, self.pair_dist_global_max))

        if d_min > d_max:
            d_min, d_max = d_max, d_min

        # Keep a minimum span to avoid exploding gradients from near-zero windows.
        if (d_max - d_min) < self.pair_dist_min_span:
            mid = 0.5 * (d_min + d_max)
            half = 0.5 * self.pair_dist_min_span
            d_min = max(self.pair_dist_global_min, mid - half)
            d_max = min(self.pair_dist_global_max, mid + half)
            if d_max - d_min < self.pair_dist_min_span:
                d_min = max(self.pair_dist_global_min, d_max - self.pair_dist_min_span)
                d_max = min(self.pair_dist_global_max, d_min + self.pair_dist_min_span)
    ...
    def gradients(self, x_start, x, sampling_info):
        with th.enable_grad():
            ...
            current_idx = sampling_info.get("current_idx", None)
            all_starts = sampling_info.get("all_starts", None)
            pair_dists = sampling_info.get("pair_dists", None)
            active_pair_cnt = 0

            if isinstance(current_idx, int) and isinstance(all_starts, list):
                for other_idx, y_start in enumerate(all_starts):
                    if other_idx == current_idx or y_start is None:
                        continue
                    ...
                    try:
                        if (
                            isinstance(pair_dists, list)
                            and current_idx < len(pair_dists)
                            and isinstance(pair_dists[current_idx], list)
                            and other_idx < len(pair_dists[current_idx])
                        ):
                            pair_range = pair_dists[current_idx][other_idx]
                            if isinstance(pair_range, list) and len(pair_range) == 2:
                                d_min = float(pair_range[0])
                                d_max = float(pair_range[1])
                    except Exception:
                        d_min, d_max = None, None

                    # Backward compatibility for static graphs without pair ranges.
                    if d_min is None or d_max is None:
                        if "unrelated_idx" in sampling_info and isinstance(sampling_info["unrelated_idx"], list):
                            if other_idx in sampling_info["unrelated_idx"]:
                                d_min, d_max = 0.8, None
                            else:
                                continue
                        else:
                            continue

                    d_min, d_max = self._sanitize_pair_range(d_min, d_max)
                    if d_min is None and d_max is None:
                        continue
                    ...
                    distances = torch.norm((root_pos1 - root_pos2), dim=-1)

                    if d_min is not None:
                        low_violation = torch.relu((d_min - self.pair_dist_soft_margin) - distances)
                        low_violation = torch.clamp(low_violation, max=self.pair_dist_violation_cap)
                        losses += (low_violation * low_violation).sum()
                    if d_max is not None:
                        high_violation = torch.relu(distances - (d_max + self.pair_dist_soft_margin))
                        high_violation = torch.clamp(high_violation, max=self.pair_dist_violation_cap)
                        losses += (high_violation * high_violation).sum()
                    active_pair_cnt += 1

            if sampling_info['t'] % 50 == 0:
                print('distance_range_loss: ', losses, 'active_pairs:', active_pair_cnt)
    ...
    def gradients_ddim(self, x_start, x):
        with th.enable_grad():
            # No topology-guided loss in DDIM control path.
            return 0, 0
